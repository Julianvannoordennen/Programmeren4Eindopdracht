  ...
  maakNieuweMaaltijd(req, res, next) {
    ...
    authentication.decodeToken(token, (err, payload) => {
      if (err) {
        ...
        try { assert(typeof huisId === "number", "het id van het huis moet een getal zijn.")
        } catch (ex) {
        ...
        let query = {
          sql: "INSERT INTO maaltijd VALUES (?, ?, ?, ?, ?, ?, ?, ?) WHERE StudentenhuisId = ?",
          values: [
            rows.insertId,
            maaltijd.naam,
...
verwijderMaaltijd(req, res, next) {
  ...
  let selectQuery = {
    sql:
      "SELECT (ID, UserID) FROM maaltijd WHERE StudentenHuisId = ? AND ID = ?",
    values: [huisId, maaltijdId],
    timeout: 2000
  }
